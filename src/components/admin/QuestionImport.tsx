...
export default function QuestionImport({ onImportSuccess }: QuestionImportProps) {
  ...
  const downloadTemplate = () => {
    ...
    const fileName = `template-${selectedExam.title.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()}.xlsx`;
    XLSX.writeFile(wb, fileName);

    toast.success(`Template Excel untuk "${selectedExam.title}" berhasil didownload!`);
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-[95vw] sm:max-w-2xl max-h-[95vh] overflow-auto">
            <div className="p-3 sm:p-6">
              ...
              {/* Exam Selection */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Pilih Ujian Target</label>
                <select
              ...
              {/* File Upload Area */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Upload File Soal</label>
                <div
                ...
                >
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-2">Drag & drop file CSV/Excel di sini, atau klik untuk pilih file</p>
                  <p className="text-sm text-gray-500 dark:text-gray-500 mb-4">Format yang didukung: .csv, .xlsx, .xls</p>
                  <input type="file" accept=".csv,.xlsx,.xls" onChange={handleFileInputChange} className="hidden" id="file-upload" disabled={uploading} />
                  <label
                    htmlFor="file-upload"
